(() => {
  ...
  const mobileMode=()=>matchMedia('(pointer:coarse)').matches||innerWidth<800;
  const actorScale=()=>mobileMode()?.70:1;
  const arenaTop=()=>mobileMode()?120:145;
  const arenaBottom=()=>mobileMode()?Math.max(360,h-138):h-30;

  function resize(){
    dpr=Math.min(devicePixelRatio||1,2); w=innerWidth; h=innerHeight;
    canvas.width=Math.round(w*dpr); canvas.height=Math.round(h*dpr);
    ctx.setTransform(dpr,0,0,dpr,0,0);
    if(player){player.x=clamp(player.x,34,w-34);player.y=clamp(player.y,arenaTop()+5,arenaBottom()-26);}
  }
  ...
  function spawnEnemy(type='cop'){
    ...
    const bottom=arenaBottom();
    const top=arenaTop();
    if(side===0){x=rnd(pad,w-pad);y=top}else if(side===1){x=w-pad;y=rnd(top+20,bottom-pad)}else if(side===2){x=rnd(pad,w-pad);y=bottom-pad}else{x=pad;y=rnd(top+20,bottom-pad)}
    const baseSpeed=type==='dog'?(level===2?140:126):rnd(cfg.copMin,cfg.copMax);
    enemies.push({x,y,type,r:type==='dog'?20:23,speed:baseSpeed*(mobile?.76:1),phase:rnd(0,6.2),yield:0});
  }
  function spawnPickup(){
    const r=Math.random(),type=r<.62?'star':r<.82?'freeze':'boost';
    pickups.push({x:rnd(55,w-55),y:rnd(arenaTop()+45,arenaBottom()-45),type,r:18,life:12,bob:rnd(0,6.2)});
  }
  function reset(){
    const cfg=levels[level];
    const mobile=mobileMode();
    elapsed=stars=score=freeze=boost=toastTimer=oneWayUntil=0;nextPickup=1.5;paused=false;
    player={x:w/2,y:arenaTop()+(arenaBottom()-arenaTop())*.58,r:22,speed:cfg.playerSpeed*(mobile?1.13:1)};enemies=[];pickups=[];confetti=[];
    const startingCops=mobile?Math.max(2,cfg.cops-1):cfg.cops;
    for(let i=0;i<startingCops;i++)spawnEnemy();
    freeze=mobile?3:0; updateUi();
  }
  function start(){reset();state='play';document.querySelector('.game-shell').classList.add('playing');ui.startPanel.classList.remove('open');ui.endPanel.classList.remove('open');ui.paused.classList.remove('show');last=performance.now();beep(620,.12);}
  ...
  function update(dt){
    ...
    const speed=player.speed*(boost>0?1.42:1);player.x=clamp(player.x+dx/len*speed*dt,30,w-30);player.y=clamp(player.y+dy/len*speed*dt,arenaTop()+5,arenaBottom()-25);
    const startingCops=mobile?Math.max(2,cfg.cops-1):cfg.cops;
    const backupAt=cfg.backupAt+(mobile?7:0),dogAt=cfg.dogAt+(mobile?7:0);
    if(elapsed>backupAt&&enemies.filter(e=>e.type==='cop').length===startingCops){spawnEnemy();showToast('🚨 BACKUP ARRIVED!')}
    if(elapsed>dogAt&&!enemies.some(e=>e.type==='dog')){spawnEnemy('dog');showToast('🐕 POLICE DOG!')}
    routeDirector();
    enemies.forEach(e=>{
      ...
      const ramp=1+elapsed/180;e.x=clamp(e.x+vx/dist*e.speed*ramp*dt,22,w-22);e.y=clamp(e.y+vy/dist*e.speed*ramp*dt,arenaTop(),arenaBottom()-22);
      const hitDistance=(e.r+player.r)*actorScale()-(mobile?6:7);
      if(Math.hypot(e.x-player.x,e.y-player.y)<hitDistance)finish(false,e.type);
    });
  ...
  function actorLabel(text,x,y,color){const small=mobileMode();ctx.save();ctx.font=`800 ${small?9:11}px Inter,sans-serif`;ctx.textAlign='center';ctx.fillStyle='#fff';ctx.strokeStyle='#123454';ctx.lineWidth=small?3:4;ctx.strokeText(text,x,y);ctx.fillStyle=color;ctx.fillText(text,x,y);ctx.restore()}
  function drawPlayer(p){
    const scale=actorScale();ctx.save();ctx.translate(p.x,p.y);ctx.scale(scale,scale);ctx.shadowColor='#184b6b66';ctx.shadowBlur=8;ctx.shadowOffsetY=5;
    rounded(0,8,27,'#fff');rounded(0,5,23,boost>0?'#ffe552':'#ff6c54');
    ctx.shadowColor='transparent';rounded(0,-10,8,'#ffd0a8');ctx.strokeStyle='#123454';ctx.lineWidth=6;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(0,-1);ctx.lineTo(0,13);ctx.moveTo(0,4);ctx.lineTo(-14,10);ctx.moveTo(0,4);ctx.lineTo(14,-2);ctx.moveTo(0,13);ctx.lineTo(-11,24);ctx.moveTo(0,13);ctx.lineTo(12,23);ctx.stroke();ctx.restore();actorLabel('YOU',p.x,p.y-35*scale,'#123454');
  }
  function drawCop(e){
    const scale=actorScale();ctx.save();ctx.translate(e.x,e.y);ctx.scale(scale,scale);ctx.shadowColor='#173e5b66';ctx.shadowBlur=7;ctx.shadowOffsetY=4;rounded(0,5,27,'#fff');rounded(0,4,22,'#3478d4');ctx.shadowColor='transparent';rounded(0,-8,8,'#f2bd92');ctx.fillStyle='#174e94';ctx.fillRect(-11,-18,22,7);ctx.fillStyle='#ffe552';ctx.fillRect(-3,-20,6,5);ctx.fillStyle='#fff';ctx.font='900 13px Inter';ctx.textAlign='center';ctx.fillText('★',0,9);ctx.restore();actorLabel('POLICE',e.x,e.y-35*scale,'#174e94');
  }
  function drawDog(e){
    const scale=actorScale();ctx.save();ctx.translate(e.x,e.y);ctx.scale(scale,scale);ctx.shadowColor='#173e5b66';ctx.shadowBlur=7;ctx.shadowOffsetY=4;rounded(0,5,25,'#fff');rounded(0,5,20,'#c97a2e');ctx.fillStyle='#70411e';ctx.beginPath();ctx.moveTo(-16,-7);ctx.lineTo(-24,-20);ctx.lineTo(-6,-13);ctx.moveTo(16,-7);ctx.lineTo(24,-20);ctx.lineTo(6,-13);ctx.fill();rounded(-7,1,3,'#172e3e');rounded(7,1,3,'#172e3e');rounded(0,10,4,'#172e3e');ctx.restore();actorLabel('DOG',e.x,e.y-31*scale,'#9b4d18');
  }
  function draw(){
    const bottom=arenaBottom(),arenaHeight=bottom-125;
    const mobile=mobileMode(),decorSize=mobile?21:28,pickupSize=mobile?23:28,pickupRadius=mobile?20:24;
    ctx.clearRect(0,0,w,h);ctx.fillStyle=level===2?'#66d9a0':'#84df60';ctx.fillRect(0,0,w,bottom);
    // Sunny plaza and open paths
    const crossY=125+arenaHeight*.48;
    ctx.fillStyle=level===2?'#ffcb7a':'#f8df9a';ctx.fillRect(w*.42,125,w*.16,arenaHeight);ctx.fillRect(0,crossY,w,arenaHeight*.18);
    ctx.fillStyle=level===2?'#ffe3a8':'#fff1bd';ctx.fillRect(w*.465,125,w*.07,arenaHeight);ctx.fillRect(0,crossY+arenaHeight*.055,w,arenaHeight*.07);
    // Park features kept near edges so the chase area stays readable.
    const pondRadiusY=mobile?34:45,pondRadiusX=Math.min(mobile?70:85,w*(mobile?.09:.1));
    ctx.fillStyle=level===2?'#f06d75':'#43bde9';ctx.strokeStyle='#fff';ctx.lineWidth=mobile?4:5;ctx.beginPath();ctx.ellipse(w*.18,125+arenaHeight*.22,pondRadiusX,pondRadiusY,0,0,Math.PI*2);ctx.fill();ctx.stroke();
    const stallWidth=Math.min(mobile?115:150,w*(mobile?.16:.18)),stallHeight=mobile?42:55;
    ctx.fillStyle='#ffe663';ctx.fillRect(w*.72,125+arenaHeight*.15,stallWidth,stallHeight);ctx.fillStyle='#ff6f6f';ctx.fillRect(w*.72,125+arenaHeight*.15-(mobile?9:12),stallWidth,mobile?14:18);
    for(let i=0;i<18;i++){const x=(i%6)*(w/5.5)+30,y=170+Math.floor(i/6)*Math.max(80,(arenaHeight-100)/2);emoji(level===2?(i%3?'🧺':'⛱️'):(i%3?'🌳':'🌸'),x,y,decorSize)}
    ctx.strokeStyle='#ffffff99';ctx.lineWidth=4;ctx.setLineDash([12,12]);ctx.strokeRect(18,137,w-36,bottom-155);ctx.setLineDash([]);
    pickups.forEach(p=>{rounded(p.x,p.y+3,pickupRadius,'#ffffffcc');emoji(p.type==='star'?'⭐':p.type==='freeze'?'❄️':'👟',p.x,p.y+Math.sin(p.bob)*4,pickupSize)});
    enemies.forEach(e=>e.type==='dog'?drawDog(e):drawCop(e));
    if(player)drawPlayer(player);
    if(freeze>0){ctx.fillStyle='#b9f4ff55';ctx.fillRect(0,135,w,bottom-135)}
    if(mobileMode()){ctx.fillStyle='#164f75';ctx.fillRect(0,bottom,w,h-bottom);ctx.fillStyle='#8be3ff';ctx.font='800 10px Inter';ctx.textAlign='left';ctx.fillText('MOVE CONTROLS',18,bottom+24)}
  }
  ...
  document.querySelector('.game-shell').addEventListener('contextmenu',event=>event.preventDefault());
